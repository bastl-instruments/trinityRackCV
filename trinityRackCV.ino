...
void reflectValueChange(uint8_t channel, uint8_t value){
	...
	case LFO_MODE:
		...
		/*
		if( bitRead(channelValue[channel][SETTINGS_BYTE],CV_ASSING_1) ){
			form+=hardware.getCVValue(channel);
		}
		*/
		...
		if(channelSync[channel]){
			...
			if(syncFast[channel]) LFO[channel].setPeriodInSteps(clockInDuration/syncFactor[channel]),buffer[channel].setPeriodInSteps(clockInDuration/syncFactor[channel]);
			else LFO[channel].setPeriodInSteps(clockInDuration*syncFactor[channel]),buffer[channel].setPeriodInSteps(clockInDuration*syncFactor[channel]);

		}
		else{
		//	LFO[channel].setPeriodInSteps((256-channelValue[channel][0])<<6);
			LFO[channel].setPeriodInSteps(curveMap(255-rate,LFOMAP_POINTS,LFOMap));
			buffer[channel].setPeriodInSteps(curveMap(255-rate,LFOMAP_POINTS,LFOMap));
		}
		...
		case 0:
			LFO[channel].setWaveform(SAW,false,false,FOLDING);
			wshape=false;
			randomLfo[channel]=false;
			break;
		case 1:
			LFO[channel].setWaveform(TRIANGLE,false,false,FOLDING);
			wshape=false;
			randomLfo[channel]=false;
			break;
		case 2:
			LFO[channel].setWaveform(SAW,true,false,FOLDING);
			wshape=true;
			randomLfo[channel]=false;
			break;
		case 3:
			LFO[channel].setWaveform(SAW,false,true,FOLDING);
			wshape=false;
			randomLfo[channel]=false;
			break;
		case 4:
			LFO[channel].setWaveform(TRIANGLE,false,true,FOLDING);
			wshape=false;
			randomLfo[channel]=false;
			break;
		case 5:
			LFO[channel].setWaveform(SAW,true,true,FOLDING);
			wshape=false;
			randomLfo[channel]=false;
			break;
		case 6:
			...
			LFO[channel].setWaveform(RANDOM,false,false,FOLDING);
...
void clockInCall(){
	...
	for(int i=0;i<6;i++){
		...
		if(channelMode[i]==INTERPOLATE_MODE){
			buffer[i].setPeriodInSteps(clockInDuration);
		...
		switch(channelMode[i]){
			...
			case LFO_MODE:
				...
				if(randomLfo[i]){
					...
					LFO[i].step();
					if(wshape){
						out=LFO[i].getCurrentValue()>>(form>>5);
						out=out<<(form>>5);
					}
					else{
						out=LFO[i].getCurrentValue()^(form>>1);
					}
				}
				hardware.setDAC(i,out);
				break;
